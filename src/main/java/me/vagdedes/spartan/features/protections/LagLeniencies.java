...
import me.vagdedes.spartan.objects.system.LagLeniency;
...
public class LagLeniencies {
    ...
    public static boolean hasPingExemption(SpartanPlayer p) {
        ...
        if (!pingException) {
            Set<Map.Entry<Enums.HackType, Integer>> set = Check.getViolationsEntry(p.getUniqueId());

            if (set.size() > 0) {
                int cancelViolations = 0;
                ResearchEngine.DataType dataType = p.getDataType();

                for (Map.Entry<Enums.HackType, Integer> entry : set) {
                    if (entry.getValue() >= CancelViolation.get(entry.getKey(), dataType)) {
                        cancelViolations += 1;
    ...
    public static double getDelay(SpartanPlayer p) {
        ...
        if (!Latency.canUseProtection() || hasPingExemption(p)) {
            ...
            if (latency <= goodPing) {
                ...
                pingDelay = Math.min(
                        ...
                        Math.min(Settings.getInteger("Protections.max_supported_player_latency"), maxAllowedPing)
                );
            }
        }
        ...
        if (!Settings.getBoolean(Settings.tpsProtectionOption)) {
            tpsDelay = 0.0;
        } else {
    ...
    public static boolean hasInconsistencies(SpartanPlayer p, String key) {
        ...
        if (key == null) {
            boolean tps = Settings.getBoolean(Settings.tpsProtectionOption),
